...
public class CustomDatePicker extends FrameLayout {
    ...
    private boolean scrollToNextMonthYearEnabled = true;

    private String[] monthNames;

    private int selectorDrawable = R.drawable.selector_drawable;
    private int numberPickerTheme = R.style.numberPickerTheme;

    public CustomDatePicker(ViewGroup root, int _numberPickerStyle, int _selectorDrawable) {
        super(root.getContext());
        mContext = root.getContext();
        if (_selectorDrawable > 0) {
            this.selectorDrawable = _selectorDrawable;
        }
        if (_numberPickerStyle > 0) {
            this.numberPickerTheme = _numberPickerStyle;
        }
        ...
        LayoutInflater inflater = (LayoutInflater) new ContextThemeWrapper(mContext,
                numberPickerTheme).getSystemService(
        ...
        // day
        View mDaySpinnerParent = inflater.inflate(R.layout.number_picker_day_month,
                mPickerContainer, false);
        mDaySpinner = (NumberPicker) mDaySpinnerParent.findViewById(R.id.number_picker);
        mDaySpinner.setId(View.generateViewId());
        mDaySpinner.setFormatter(new TwoDigitFormatter());
        mDaySpinner.setOnLongPressUpdateInterval(100);
        mDaySpinner.setOnValueChangedListener(onChangeListener);
        mDaySpinnerInput = NumberPickers.findEditText(mDaySpinner);

        View viewSelectedBackgroundDay = mDaySpinnerParent.findViewById(R.id.viewSelectedBackground);
        viewSelectedBackgroundDay.setBackgroundResource(selectorDrawable);

        // month
        View mMonthSpinnerParent = inflater.inflate(R.layout.number_picker_day_month,
                mPickerContainer, false);
        mMonthSpinner = (NumberPicker) mMonthSpinnerParent.findViewById(R.id.number_picker);
        mMonthSpinner.setId(View.generateViewId());
        ...
        View viewSelectedBackgroundMonth = mMonthSpinnerParent.findViewById(R.id.viewSelectedBackground);
        viewSelectedBackgroundMonth.setBackgroundResource(selectorDrawable);


        // year
        View mYearSpinnerParent = inflater.inflate(R.layout.number_picker_year,
                mPickerContainer, false);
        mYearSpinner = (NumberPicker) mYearSpinnerParent.findViewById(R.id.number_picker);
        mYearSpinner.setId(View.generateViewId());
        mYearSpinner.setOnLongPressUpdateInterval(100);
        mYearSpinner.setOnValueChangedListener(onChangeListener);
        mYearSpinnerInput = NumberPickers.findEditText(mYearSpinner);
        View viewSelectedBackgroundYear = mYearSpinnerParent.findViewById(R.id.viewSelectedBackground);
        viewSelectedBackgroundYear.setBackgroundResource(selectorDrawable);
    ...
    public CustomDatePicker(ViewGroup root, int numberPickerStyle) {
        this(root, numberPickerStyle, R.drawable.selector_drawable);
    }

    public CustomDatePicker(ViewGroup root) {
        this(root, R.style.numberPickerTheme, R.drawable.selector_drawable);
    }
    ...
    public void init(int year, int monthOfYear, int dayOfMonth,
                     boolean isDayShown, String[] monthNames) {
        this.monthNames = monthNames;
        mShortMonths = monthNames;
        mIsDayShown = isDayShown;
        setDate(year, monthOfYear, dayOfMonth);
...
////        String[] a = {"jan.", "feb.", "mar.", "abr.", "may.", "jun.", "jul.", "aug.", "sept.", "oct.", "nov.", "dec."};
//        String[] a = getResources().getStringArray(R.array.months_array_hi);
//        return a;
//    }
...
//        char[] order = ICU.getDateFormatOrder(pattern);
        ...
        for (int i = 0; i < spinnerCount; i++) {
            switch (order[i]) {
                case 'd':
                    mPickerContainer.addView((View) mDaySpinner.getParent());
                    setImeOptions(mDaySpinner, spinnerCount, i);
                    break;
                case 'M':
                    mPickerContainer.addView((View) mMonthSpinner.getParent());
                    setImeOptions(mMonthSpinner, spinnerCount, i);
                    break;
                case 'y':
                    mPickerContainer.addView((View) mYearSpinner.getParent());
    ...
    private void updateSpinnersWithoutScrollToNext() {
...
//            mDaySpinner.setMaxValue(mCurrentDate.getActualMaximum(Calendar.DAY_OF_MONTH));
            if (isDateEqual(mCurrentDate, mMaxDate)) {
...
//            mMonthSpinner.setMaxValue(mCurrentDate.getActualMaximum(Calendar.MONTH));
            mMonthSpinner.setMaxValue(getMaxMonthValue());
...
//            mMonthSpinner.setMinValue(mCurrentDate.getActualMinimum(Calendar.MONTH));
            mMonthSpinner.setMinValue(getMinMonthValue());
...
//                mMonthSpinner.setMaxValue(11);
                ...
                mDaySpinner.setMinValue(1);
                mDaySpinner.setMaxValue(mMaxDate.get(Calendar.DAY_OF_MONTH));
                mDaySpinner.setWrapSelectorWheel(false);
                mMonthSpinner.setDisplayedValues(null);
//                mMonthSpinner.setMinValue(mCurrentDate.getActualMinimum(Calendar.MONTH));
                mMonthSpinner.setMinValue(getMinMonthValue());
    ...
    private int getMaxMonthValue() {
        if (isMonthAndYearEqual(mCurrentDate, mMaxDate)) {
            return mMaxDate.get(Calendar.MONTH);
        }
        return 11;
    }

    private int getMinMonthValue() {
        if (isMonthAndYearEqual(mCurrentDate, mMinDate)) {
    ...
    private void updateSpinners() {
        if (!scrollToNextMonthYearEnabled) {
            updateSpinnersWithoutScrollToNext();
            return;
        }
        // set the spinner ranges respecting the min and max dates
        mDaySpinner.setVisibility(mIsDayShown ? View.VISIBLE : View.GONE);
//        if (mCurrentDate.equals(mMinDate)) {
    ...
    private static class SavedState extends BaseSavedState {
        ...
        /**
         * Constructor called from { com.tsongkha.spinnerdatepicker.DatePicker#onSaveInstanceState()}
         */
